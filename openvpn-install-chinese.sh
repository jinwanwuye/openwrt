	...
	if [[ -e /etc/debian_version ]]; then
		...
		if [[ $ID == "debian" || $ID == "raspbian" ]]; then
			...
			if [[ $MAJOR_UBUNTU_VERSION -lt 16 ]]; then
				...
				echo "请注意他们不被支持"
				echo "但是，如果你使用的Ubuntu系统版本 > 16.04 , 你可以继续安装."
				echo ""
				until [[ $CONTINUE =~ (y|n) ]]; do
					read -rp "是否继续? [y/n]: " -e CONTINUE
				done
		...
		if [[ $ID == "centos" || $ID == "rocky" || $ID == "almalinux" ]]; then
			...
			if [[ ${VERSION_ID%.*} -lt 7 ]]; then
				...
				echo ""
				echo "此脚本只能在 CentOS 7 and CentOS 8 中运行."
				echo ""
				exit 1
			fi
		fi
		...
		echo "您似乎没有在 Debian、Ubuntu、Fedora、CentOS、Amazon Linux 2、Oracle Linux 8 或 Arch Linux 系统上运行此安装程序."
		exit 1
	fi
}
...
function installQuestions() {
	...
	if echo "$IP" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		...
		until [[ $ENDPOINT != "" ]]; do
			read -rp "公网IPv4地址或者域名: " -e -i "$PUBLICIP" ENDPOINT
		done
	fi
	...
	echo "   1) 正确的系统解析器 resolvers (from /etc/resolv.conf)"
	echo "   2) 当前host DNS 解析器  Resolver (Unbound)"
	...
	echo "请注意，无论您选择什么，脚本中提供的所有选择都是安全的。 （与 OpenVPN 的默认设置不同）."
	echo "参考 https://github.com/angristan/openvpn-install#security-and-encryption 来了解更多."
	echo ""
	until [[ $CUSTOMIZE_ENC =~ (y|n) ]]; do
		read -rp "自定义加密设置? [y/n]: " -e -i y CUSTOMIZE_ENC
	done
	if [[ $CUSTOMIZE_ENC == "n" ]]; then
		...
		CC_CIPHER="TLS-ECDHE-ECDSA-WITH-AES-128-CBC-SHA256"
		DH_TYPE="1" # ECDH
		DH_CURVE="prime256v1"
		HMAC_ALG="SHA256"
		TLS_SIG="1" # tls-crypt
		...
		until [[ $CIPHER_CHOICE =~ ^[1-6]$ ]]; do
			read -rp "加密方式 [1-6]: " -e -i 4 CIPHER_CHOICE
		done
		...
		case $CERT_TYPE in
		1)
			echo "   1) ECDHE-ECDSA-AES-128-CBC-SHA256 (recommended)"
			echo "   2) ECDHE-ECDSA-AES-256-CBC-SHA384"
			...
			1)
				...
				;;
			2)
				CC_CIPHER="TLS-ECDHE-ECDSA-WITH-AES-256-CBC-SHA384"
				;;
			esac
			;;
		2)
			echo "   1) ECDHE-RSA-AES-128-CBC-SHA256 (recommended)"
			echo "   2) ECDHE-RSA-AES-256-CBC-SHA384"
			until [[ $CC_CIPHER_CHOICE =~ ^[1-2]$ ]]; do
				read -rp"Control channel cipher [1-2]: " -e -i 1 CC_CIPHER_CHOICE
			done
			case $CC_CIPHER_CHOICE in
			1)
				CC_CIPHER="TLS-ECDHE-RSA-WITH-AES-128-CBC-SHA256"
				;;
			2)
				CC_CIPHER="TLS-ECDHE-RSA-WITH-AES-256-CBC-SHA384"
				;;
			esac
			;;
		esac
	...
	echo ""
	echo "Ok, 安装信息收集完成，现在准备好设置您的OpenVPN服务器."
...
function newClient() {
	...
	if [[ $CLIENTEXISTS == '1' ]]; then
		echo ""
		echo "设置的客户端名称已存在 easy-rsa 中，请选择其他名称."
		exit
	else
		cd /etc/openvpn/easy-rsa/ || return
		case $PASS in
